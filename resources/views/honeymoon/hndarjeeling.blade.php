...
<div class="container py-5">
    <center>
        <h2 class="text-dark fw-bold mb-3">🌟 INDIA Darjeeling TOUR 🌟</h2>
        <p class="text-muted mb-5"><i class="bi bi-geo-alt-fill"></i> India, Darjeeling</p>
    </center>
...
<div class="container mt-3" style="borer: 1px solid black">
  ...
  <img src="{{asset('hny1.jpg')}}" class="card-img-top" alt="...">
</div>
<div class="carousel mb-3">
...
<div class="container mt-3">
  ...
  <div class="carusel bg-light p-4 rounded shadow" style="resize: both; overflow: auto;">
    <div class="form">
      <form id="honeymoonform" method="POST">
        {{csrf_field()}}
...
$(document).ready(function(){
  ...
  $('#honeymoonform').submit(function(e){
    e.preventDefault();
    $.ajax({
      url: "{{ route('honeymoon.tour') }}",
      type: "POST",
      data: $(this).serialize(),
  ...
  function premiumTours(){
    $.get("{{ route('honeymoon.tours') }}", function(data){
...
$('#paymentform').on('submit', function(e) {
    ...
    $.ajax({
        type: 'POST',
        url: "{{ route('honeymoon-payment.store') }}",
        data: formData,
        contentType: false,
        processData: false,
        success: function(response) {
...
function loadPremiumPayments() {
    $.get("{{ route('honeymoon.honeymoons') }}", function (data) {
        $('#tourTableContainer').html(data);
    });
}
</script>
