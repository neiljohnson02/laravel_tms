...
class AsiapymentController extends Controller
{
     public function store(Request $request)
    {
        ...
        try {
            $payment = Asiapayment::create($data);
    ...
    {
        $payment = Asiapayment::latest()->get();
        return view('dashboard.asiapayment', compact('payment'));
    }

    // Send payment confirmation email
    public function sendMail($id)
    {
        $tour = Asiapayment::findOrFail($id);
        ...
        try {
            Mail::to($data['email'])->send(new AsiapymentMail($data));
