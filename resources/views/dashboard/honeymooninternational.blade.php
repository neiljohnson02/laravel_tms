...
<body>
  ...
  <center>
    <h5 class="pt-4">
      <i class="bi bi-stars text-warning me-2"></i> Welcome to <span class="text-dark">GT HOLIDAYS</span> PremiumTour Form Submissions
    </h5>
  </center>
  <!-- search box -->
        ...
        <tr>
            <tbody>
                @forelse($tour as $tours)
                  ...
                  <!-- Delete -->
                 <form action="{{ route('honeymooninternational.destroy', $tours->id) }}" method="POST" style="display:inline;" class="delete-form">
                    @csrf
                    @method('DELETE')
                  ...
                  <!-- Email -->
                  <form action="{{ route('honeymooninternational.email', $tours->id) }}" method="POST" style="display:inline;">
                    @csrf
