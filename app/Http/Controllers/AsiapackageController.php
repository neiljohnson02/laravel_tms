...
use App\Mail\AsiapackageMail;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Mail;
use Illuminate\Support\Facades\Validator;

class AsiapackageController extends Controller
{
     public function store(Request $request)
{
 ...
 $validated['addons'] = isset($validated['addons']) ? json_encode($validated['addons']) : null;
    ...
    $tour = Asiapackage::create($request->all());
    ...
    {
        $tour =  Asiapackage::latest()->get();
        return view('dashboard.africapackage', compact('tour')); 
    }

    public function sendMail($id)
    {
       $tour =  Asiapackage::findOrFail($id);
       $data = [
    ...
    try {
    Mail::to($data['email'])->send(new AsiapackageMail($data));
