...
<div class="container py-5">
    <center>
        <h2 class="text-dark fw-bold mb-3">🌟SOUTHAFRICA CAPETOWN TOUR 🌟</h2>
        <p class="text-muted mb-5"><i class="bi bi-geo-alt-fill"></i>SouthAfrica, CAPETOWN</p>
    </center>
...
<div class="container mt-3" style="borer: 1px solid black">
  ...
  <img src="{{asset('packafrica1.jpg')}}" style="height: 820px" class="card-img-top" alt="...">
</div>
<div class="carousel mb-3">
...
<div class="container mt-3">
  ...
  <div class="carusel bg-light p-4 rounded shadow" style="resize: both; overflow: auto;">
    <div class="form">
      <form id="europeform" method="POST">
        {{csrf_field()}}
      ...
      <!-- Premium Package -->
        <div class="form-group mb-3 col-md-8 ms-5">
          <label for="package" class="form-label">
            <i class="bi bi-gem"></i> Asia package 
          </label>
          <select name="package" class="form-control" required>
           <option value="barcelona_spain">Barcelona, Spain – 25k</option>
            <option value="realmadrid_spain">Real Madrid, Spain – 26k</option>
            <option value="rotterdam_netherlands">Rotterdam, Netherlands – 24k</option>
            <option value="thehague_netherlands">The Hague, Netherlands – 23k</option>
            <option value="paris_france">France, Paris – Eiffel Tower – 30k</option>
            <option value="louvre_france">France, Louvre Museum – 28k</option>
            <option value="statueofliberty_usa">USA, Statue of Liberty – 27k</option>
            <option value="mountrushmore_usa">USA, Mount Rushmore National Memorial – 29k</option>
            <option value="munich_germany">Germany, Munich – 26k</option>
            <option value="berlin_germany">Germany, City of Berlin – 27k</option>
            <option value="venice_italy">Italy, Venice – 25k</option>
            <option value="rome_italy">Italy, Rome – 26k</option>
            <option value="cathedral_belgium">Cathedral of Our Lady, Belgium – 22k</option>
            <option value="liseberg_sweden">Liseberg, Sweden – 24k</option>
            <option value="malmo_sweden">Malmo, Sweden – 23k</option>
            <option value="littlemermaid_denmark">The Little Mermaid, Denmark – 22k</option>
            <option value="belem_portugal">Belem, Portugal – 21k</option>
            <option value="caperoca_portugal">Cape Roca, Portugal – 22k</option>
            <option value="budapest_hungary">Budapest, Hungary – 23k</option>
            <option value="moscow_russia">Russia, Moscow – 28k</option>
          </select>
...
$(document).ready(function(){
  ...
  $('#europeform').submit(function(e){
    e.preventDefault();
    $.ajax({
      url: "{{ route('packeurope.tour') }}",
      type: "POST",
      data: $(this).serialize(),
  ...
  function premiumTours(){
    $.get("{{ route('packeurope.tours') }}", function(data){
...
$('#paymentform').on('submit', function(e) {
    ...
    $.ajax({
        type: 'POST',
        url: "{{ route('packeurope-payment.store') }}",
        data: formData,
        contentType: false,
        processData: false,
        success: function(response) {
...
function loadPremiumPayments() {
    $.get("{{ route('packeurope.packeuropes') }}", function (data) {
        $('#tourTableContainer').html(data);
    });
}
</script>
