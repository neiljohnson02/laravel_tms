...
use App\Mail\GroupofTourMail;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Mail;
use Illuminate\Support\Facades\Validator;

class GroupofTourController extends Controller
{
    ...
    $validated['addons'] = isset($validated['addons']) ? json_encode($validated['addons']) : null;
        ...
        $tour = GroupofTour::create($request->all());
        ...
        {
            $tour = GroupofTour::latest()->get();
            
            return view('dashboard.groupoftours', compact('tour')); 
        }


        public function static(){
             $groupTourCount = GroupofTour::count();
              return view('dashboard.admin', compact('groupTourCount')); 

        }

        public function sendMail($id)
        {
        $tour = GroupofTour::findOrFail($id);
        $data = [
        ...
        try {
        Mail::to($data['email'])->send(new GroupofTourMail($data));
