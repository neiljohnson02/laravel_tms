...
<body>
  ...
  <center>
    <h5 class="pt-4">
      <i class="bi bi-stars text-warning me-2"></i> Welcome to <span class="text-dark">GT HOLIDAYS</span> Trending-Tour Form Submissions
    </h5>
  </center>
  <!-- search box -->
          ...
          <table class="table table-hover table-striped align-middle text-center">
            <thead>
              <tr>
                ...
                <th><i class="bi bi-globe2"></i> Tour Type</th>
                ...
                @forelse($tour as $tours)
                    <tr>
                     <td><i class="bi bi-hash text-secondary"></i> <span class="fw-bold text-muted">{{ $tours->id }}</span></td>
                      <td><i class="bi bi-person-fill text-primary"></i> {{ $tours->name }}</td>
                      <td><i class="bi bi-globe2 text-info"></i> {{ $tours->tourtype }}</td>
                      <td><i class="bi bi-calendar-check-fill text-success"></i> {{ $tours->departure }}</td>
                      ...
                      <td><i class="bi bi-baby text-pink"></i> {{ $tours->infant }}</td>
                      <td><i class="bi bi-envelope-fill text-danger"></i> {{ $tours->email }}</td>
                      <td><i class="bi bi-telephone-fill text-success"></i> {{ $tours->phone }}</td>

                        <td>
                  <!-- Delete -->
                 <form action="{{ route('trendingtour.destroy', $tours->id) }}" method="POST" style="display:inline;" class="delete-form">
                    @csrf
                    @method('DELETE')
                  ...
                  <!-- Email -->
                  <form action="{{ route('trendingtour.email', $tours->id) }}" method="POST" style="display:inline;">
                    @csrf
