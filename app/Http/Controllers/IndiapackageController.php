...
use App\Mail\IndiapackageMail;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Mail;
use Illuminate\Support\Facades\Validator;

class IndiapackageController extends Controller
{
     public function store(Request $request)
{
 ...
 $validated['addons'] = isset($validated['addons']) ? json_encode($validated['addons']) : null;
    ...
    $tour = Indiapackage::create($request->all());
    ...
    {
        $tour = Indiapackage::latest()->get();
      
        return view('dashboard.indiapackage', compact('tour')); 
    }

    public function sendMail($id)
    {
       $tour = Indiapackage::findOrFail($id);
       $data = [
    ...
    try {
    Mail::to($data['email'])->send(new IndiapackageMail($data));
