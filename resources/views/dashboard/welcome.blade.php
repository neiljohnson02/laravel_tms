...
<body>
  ...
  <center>
    <h5 class="pt-4">
      <i class="bi bi-stars text-warning me-2"></i> Welcome to <span class="text-dark">GT HOLIDAYS</span> Form Submissions
    </h5>
  </center>
  <!-- search box -->
          ...
          <table class="table table-hover table-striped align-middle text-center">
            <thead>
              <tr>
               <th><i class="bi bi-hash"></i> ID</th>
                ...
                <th><i class="bi bi-geo-alt-fill"></i> City</th>
                <th><i class="bi bi-envelope-fill"></i> Email</th>
                <th><i class="fas fa-map"></i> Tourtype</th>
                <th><i class="bi bi-telephone-fill"></i> Phone</th>
                <th><i class="bi bi-whatsapp"></i> WhatsApp</th>
                <th><i class="bi bi-airplane-fill"></i> Destination</th>
                <th><i class="bi bi-calendar-event-fill"></i> Travel Date</th>
                <th><i class="bi bi-people-fill"></i> Travelers</th>
                <th><i class="bi bi-map-fill"></i> Vocation Type</th>
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><span class="badge bg-light text-dark">{{ $row->cityofresitense }}</span></td>
                <td class="text-dark"><i class="bi bi-envelope-paper-fill me-1"></i>{{ $row->email }}</td>
                <td><i class="fas fa-globe"></i> {{ $row->tourtype }}</td>
                <td>{{ $row->phonenumber }}</td>
                <td class="text-success"><i class="bi bi-whatsapp"></i> {{ $row->whatsapp }}</td>
                <td><i class="bi bi-airplane-engines-fill text-info"></i> {{ $row->traveldestination }}</td>
                <td><span class="badge bg-warning text-dark">{{ $row->dateoftravel }}</span></td>
                <td><span class="badge bg-secondary">{{ $row->numberoftravelers }}</span></td>
                <td><span class="badge bg-info text-dark">{{ $row->vocationtype }}</span></td>
                <td>
                  <!-- Edit -->
                
                  
                  <!-- Delete -->
                 <form action="{{ route('user.destroy', $row->id) }}" method="POST" style="display:inline;" class="delete-form">
                    @csrf
                    @method('DELETE')
                  ...
                  <!-- Email -->
                  <form action="{{ route('send.email', $row->id) }}" method="POST" style="display:inline;">
                    @csrf
