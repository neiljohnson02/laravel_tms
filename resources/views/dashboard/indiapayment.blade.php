...
<body>
  ...
  <center>
    <h5 class="pt-4">
      <i class="bi bi-stars text-warning me-2"></i> Welcome to <span class="text-dark">GT HOLIDAYS</span>AFRICA-PACKAGE PAYMENT Form Submissions
    </h5>
  </center>
  <!-- search box -->
            ...
            <thead>
             ...
             <tbody>
               @foreach($payment as $tours)
                ...
                <td>
                  <!-- Delete -->
                 <form action="{{ route('packindiapayment.destroy', $tours->id) }}" method="POST" style="display:inline;" class="delete-form">
                    @csrf
                    @method('DELETE')
                  ...
                  <!-- Email -->
                  <form action="{{ route('packindiapayment.email', $tours->id) }}" method="POST" style="display:inline;">
                    @csrf
...
<script>
 ...
 <script>
  document.addEventListener("DOMContentLoaded", function () {
    const baseCost = 50000; // Fixed cost to subtract

    document.querySelectorAll(".profit-cell").forEach(function (cell) {
      const amount = parseFloat(cell.getAttribute("data-amount"));

      if (!isNaN(amount)) {
        const profit = amount - baseCost;
        cell.textContent = '$' + profit.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2});
      } else {
