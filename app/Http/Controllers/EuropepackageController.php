...
use App\Mail\EuropepackageMail;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Mail;
use Illuminate\Support\Facades\Validator;

class EuropepackageController extends Controller
{
     public function store(Request $request)
{
 ...
 $validated['addons'] = isset($validated['addons']) ? json_encode($validated['addons']) : null;
    ...
    $tour = Europepackage::create($request->all());
    ...
    {
        $tour = Europepackage::latest()->get();
        return view('dashboard.europepackage', compact('tour')); 
    }

    public function sendMail($id)
    {
       $tour = Europepackage::findOrFail($id);
       $data = [
    ...
    try {
    Mail::to($data['email'])->send(new EuropepackageMail($data));
