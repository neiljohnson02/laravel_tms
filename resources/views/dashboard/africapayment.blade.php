...
<body>
  ...
  <center>
<h5 class="pt-4">
   <i class="bi bi-clipboard-data text-dark me-2"></i> <strong></strong> <span class="text-dark">GT HOLIDAYS</span> – AFRICA-PAYMENT DOCUMENTS
</h5>
            ...
            <thead>
             ...
             <tbody>
               @foreach($payment as $tours)
                ...
                <td>
                  <!-- Delete -->
                 <form action="{{ route('packafricapayment.destroy', $tours->id) }}" method="POST" style="display:inline;" class="delete-form">
                    @csrf
                    @method('DELETE')
                  ...
                  <!-- Email -->
                  <form action="{{ route('packafricapayment.email', $tours->id) }}" method="POST" style="display:inline;">
                    @csrf
