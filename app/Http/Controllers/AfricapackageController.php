...
use App\Mail\AfricapackageMail;
use Illuminate\Support\Facades\Log;
use Illuminate\Support\Facades\Mail;
use Illuminate\Support\Facades\Validator;

class AfricapackageController extends Controller
{
     public function store(Request $request)
{
 ...
 $validated['addons'] = isset($validated['addons']) ? json_encode($validated['addons']) : null;
    ...
    $tour = Africapackage::create($request->all());
    ...
    {
        $tour = Africapackage::latest()->get();
        return view('dashboard.africapackage', compact('tour')); 
    }

    public function sendMail($id)
    {
       $tour = Africapackage::findOrFail($id);
       $data = [
    ...
    try {
    Mail::to($data['email'])->send(new AfricapackageMail($data));
