...
use App\Mail\HoneymoonPackageMail;
...
class HoneymoonPackageController extends Controller
{
    ...
    $validated['addons'] = isset($validated['addons']) ? json_encode($validated['addons']) : null;
        ...
        $tour = HoneymoonPackage::create($request->all());
        ...
        {
            $tour = HoneymoonPackage::latest()->get();
            return view('dashboard.honeymoon', compact('tour')); 
        }

        public function sendMail($id)
        {
        $tour = HoneymoonPackage::findOrFail($id);
        $data = [
        ...
        try {
        Mail::to($data['email'])->send(new HoneymoonPackageMail($data));
        ...
        return redirect()->back()->with('success', 'User deleted successfully.');
         ...
         public function honeymoonstatic(){
             $honeymoonTourCount = HoneymoonPackage::count();
              return view('dashboard.admin', compact('honeymoonTourCount')); 
